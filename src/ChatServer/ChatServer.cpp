...
ChatServer::ChatServer() : mNetworkManager(0),mDatabaseManager(0),mRouterService(0),mDatabase(0), mLastHeartbeat(0)
{
    ...
    mDatabase->ExecuteProcedureAsync(0, 0, "CALL swganh_config.sp_ServerStatusUpdate('chat', NULL, NULL, NULL);");
    gLogger->log(LogManager::DEBUG, "CALL swganh_config.sp_ServerStatusUpdate('chat', NULL, NULL, NULL);"); // SQL Debug Log
...
void ChatServer::_updateDBServerList(uint32 status)
{
    ...
    mDatabase->ExecuteProcedureAsync(0, 0, "CALL swganh_config.sp_ServerStatusUpdate('chat', %u, '%s', %u);", status, mRouterService->getLocalAddress(), mRouterService->getLocalPort()); // SQL - Update server status
    gLogger->log(LogManager::DEBUG, "SQL :: CALL swganh_config.sp_ServerStatusUpdate('chat', %u, '%s', %u);", status, mRouterService->getLocalAddress(), mRouterService->getLocalPort()); // SQL Debug Log
}
