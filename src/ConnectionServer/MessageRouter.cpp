...
void MessageRouter::_loadMessageProcessMap(void)
{
	...
	DatabaseResult* result = mDatabase->ExecuteSynchSql("SELECT messageId, processId FROM swganh_config.config_message_routes;");
	gLogger->log(LogManager::DEBUG, "SQL :: SELECT messageId, processId FROM swganh_config.config_message_routes;"); // SQL Debug Log
